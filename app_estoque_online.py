...
def carregar_dados():
    ...
    if os.path.exists(ESTOQUE_FILE) and os.path.exists(CADASTROS_FILE):
        ...
        except (pd.errors.EmptyDataError, FileNotFoundError, json.JSONDecodeError) as e:
            st.error(f"Erro ao carregar dados: {e}. Criando arquivos de dados padrão.")
            inicializar_dados_padrao(colunas)
    else:
        inicializar_dados_padrao(colunas)

def inicializar_dados_padrao(colunas):
...
def gerar_pdf_relatorio(dataframe, titulo):
    ...
    # Definir cores para o cabeçalho da tabela
    pdf.set_fill_color(46, 46, 84)  # Azul escuro
    pdf.set_text_color(255, 255, 255)  # Branco para o texto do cabeçalho
    pdf.set_font("Arial", "B", 8)

    # Calcular largura das colunas
    col_width = (pdf.w - 2 * pdf.l_margin) / len(dataframe.columns) if len(dataframe.columns) > 0 else 0
    row_height = 8 # Altura da linha para melhor espaçamento

    # Cabeçalho da tabela
    for header in dataframe.columns:
        pdf.cell(col_width, row_height, str(header), 1, 0, "C", fill=True)
    pdf.ln()

    # Dados da tabela
    pdf.set_font("Arial", "", 8)
    pdf.set_text_color(0, 0, 0)  # Preto para o texto dos dados
    fill = False
    for _, row in dataframe.iterrows():
        # Alternar cor de fundo das linhas
        pdf.set_fill_color(230, 230, 230) if fill else pdf.set_fill_color(255, 255, 255)
        pdf.set_text_color(0, 0, 0) # Certifica que o texto dos dados é preto

        for item in row:
            # Adicionar tratamento para valores NaN/NaT
            item_str = str(item) if pd.notna(item) else ""
            # Ajustar a célula para quebrar texto se necessário
            # FPDF não tem quebra de texto automática para cell(), precisa de multi_cell()
            # Para simplificar, vamos apenas garantir que o texto não exceda a largura da célula
            # e truncar se for muito longo, ou usar multi_cell para quebrar.
            # Para este exemplo, vamos manter cell() e focar na cor.
            pdf.cell(col_width, row_height, item_str, 1, 0, "C", fill=fill)
        fill = not fill
        pdf.ln()
    
    # Definir a cor do texto de volta para branco para o rodapé ou outras informações se houver
    pdf.set_text_color(255, 255, 255) # Volta para branco para o resto do documento
...
def pagina_painel_principal():
    ...
    itens_alerta = df[df['Quantidade em Estoque'] <= df['Estoque Mínimo']].shape[0]
...
def pagina_meu_estoque():
    ...
    c2.button("Adicionar Novo Item", on_click=set_page, args=("Adicionar Item",), use_container_width=True, type="primary")
    ...
    df_editavel = st.session_state.estoque_df.copy()
    df_editavel["Excluir"] = False
    col_config = {
        "ID": st.column_config.NumberColumn(disabled=True),
        "Quantidade em Estoque": st.column_config.NumberColumn(format="%.2f", required=True),
        "Estoque Mínimo": st.column_config.NumberColumn(format="%d", required=True),
    ...
    if c1.button("Salvar Alterações", use_container_width=True, type="primary"):
        ...
        df_original.update(df_alterado)
        st.session_state.estoque_df = df_original.reset_index()
        if itens_para_excluir:
            st.session_state.estoque_df = st.session_state.estoque_df[~st.session_state.estoque_df['ID'].isin(itens_para_excluir)]
        salvar_dados()
...
def pagina_adicionar_item():
    ...
    with st.form("novo_item_form", clear_on_submit=True):
        ...
        cat=c2.selectbox("Categoria*",st.session_state.categorias)
        forn=c2.selectbox("Fornecedor Principal*",st.session_state.fornecedores)
        un=c2.text_input("Unidade de Medida*")
        c3, c4 = st.columns(2)
        pr=c3.number_input("Preço de Custo (R$)*", min_value=0.0, format="%.2f")
        qtd=c3.number_input("Quantidade em Estoque*", min_value=0.0)
        est_min=c4.number_input("Estoque Mínimo*", min_value=0)
        obs=c4.text_area("Observações Adicionais")
        if st.form_submit_button("Adicionar Item", use_container_width=True, type="primary"):
            if not all([nome, cat, forn, un]):
                st.error("Preencha os campos obrigatórios (*).")
            else:
                try:
                    # Tentar converter para os tipos corretos
                    quantidade_float = float(qtd)
                    estoque_min_int = int(est_min)
                    preco_custo_float = float(pr)
                    adicionar_item(nome,marca,especificacao,cat,forn,quantidade_float,estoque_min_int,un,preco_custo_float,obs)
                    st.success("✅ Item adicionado!")
                except ValueError:
                    st.error("Por favor, insira valores numéricos válidos para Quantidade, Estoque Mínimo e Preço de Custo.")
    ...
    with c1:
        ...
        if not df.empty:
            ...
            if item_sel != "Selecione...":
                item_id = int(item_sel.split(':')[0].replace('ID','').strip())
                qtd = st.number_input("Quantidade utilizada:", 1.0, step=1.0, format="%.2f")
                if st.button("Adicionar à Sessão"):
                    item_info = df[df['ID'] == item_id].iloc[0]
                    st.session_state.sessao_uso.append({'id': item_id, 'nome': f"{item_info['Nome do Item']} ({item_info['Marca/Modelo']})", 'qtd': qtd})
                    st.rerun()
    with c2:
...
def pagina_lista_compras():
    ...
    st.write("Itens que atingiram o estoque mínimo.")
    if (lista := gerar_lista_de_compras()) is not None:
        st.dataframe(lista, use_container_width=True, hide_index=True)
        pdf_data = gerar_pdf_relatorio(lista, "Lista de Compras")
        st.download_button("Baixar Lista PDF", pdf_data, f"lista_compras_{date.today()}.pdf", "application/pdf")
    else:
        st.success("🎉 Nenhum item precisa de reposição!")
    ...
    with c2:
        ...
        with st.form("novo_forn_form", clear_on_submit=True):
            ...
            if st.form_submit_button("Adicionar"):
                if nova and nova not in st.session_state.fornecedores:
                    st.session_state.fornecedores.append(nova)
                    salvar_dados()
                    st.rerun()
                else:
                    st.error("Inválido ou já existe.")
        if st.session_state.fornecedores:
            ...
            if st.button("Excluir Fornecedor"):
                st.session_state.fornecedores.remove(sel)
                salvar_dados()
                st.rerun()
